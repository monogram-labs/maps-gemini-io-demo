...
async function checkBuiltInPrompting() {
  if (window.model && (await window.model.canCreateGenericSession()) === 'readily') {
    console.log("Built-in prompting supported.");
    return true;
  } else {
    console.log("Built-in prompting not supported. Falling back to Gemini API.");
    return false;
  }
}
...
async function generateReviewTitle() {
    ...
    if (hasBuiltInPrompting) {
      ...
      const title = await session.execute(`Generate a catchy title for this travel review: ${reviewText} to ${placeName}`);
      promptTitle.value = title; 
      outputHTML.innerHTML = `<h2>${promptTitle.value}</h2><p>${reviewText}</p>`;
    } else {
      // Fallback to Gemini API
      const genAI = new GoogleGenerativeAI("AIzaSyA8bYErDRE8xfRAIvZJkL06l0oMp5wHVx8");
      const model = await genAI.getGenerativeModel({ model: "gemini-pro" });
      const result = await model.generateContent([`Generate a catchy title for this travel review: ${reviewText} to ${placeName}`]);
...
async function rewriteForReadability() {
    ...
    if (hasBuiltInPrompting) {
      // Use built-in prompting
      let session = await window.model.createGenericSession();
      const rewrittenText = await session.execute(`Summarize this text, first person: ${reviewText} for a trip to ${placeName}`);
      ...
      const genAI = new GoogleGenerativeAI("AIzaSyA8bYErDRE8xfRAIvZJkL06l0oMp5wHVx8");
      const model = await genAI.getGenerativeModel({ model: "gemini-pro" });
      const result = await model.generateContent([`Summarize this text, first person: ${reviewText} for a trip to ${placeName}`]);
     // promptText.value = result.response.text();
      // outputHTML.innerHTML = result.response.text();
      outputHTML.innerHTML = `<h2>${promptTitle.value}</h2><p>${result.response.text()}</p>`;
    }
  }
...
async function checkToxicity() {
    ...
    if (hasBuiltInPrompting) {
      ...
      let session = await window.model.createGenericSession();
      const toxicityLevel = await session.execute(`Analyze the toxicity level of this text: ${reviewText}`);
      ...
      const genAI = new GoogleGenerativeAI("AIzaSyA8bYErDRE8xfRAIvZJkL06l0oMp5wHVx8");
      const model = await genAI.getGenerativeModel({ model: "gemini-pro" });
      const result = await model.generateContent([`Analyze the toxicity level of this text:: ${reviewText}`]);
